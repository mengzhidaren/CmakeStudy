...
//使用这些缓冲对象的好处是我们可以一次性的发送一大批数据到显卡上
    unsigned int createVBO()
    {
        ...
        //把新创建的<缓冲buffer>绑定到GL_ARRAY_BUFFER目标上
        glBindBuffer(GL_ARRAY_BUFFER, VBO);
        //专门用来把用户定义的数据复制到当前绑定缓冲的函数->把之前定义的顶点数据复制到缓冲的内存中
        //把顶点缓冲对象当前绑定到GL_ARRAY_BUFFER目标上   参数：GL_STATIC_DRAW ：数据不会或几乎不会改变
        //把[vertices]数据 复制到缓冲的内存 给<缓冲buffer>然后上面的<缓冲buffer>VBO就有值了
...
//GLAD是用来管理OpenGL的函数指针的 所以在调用任何OpenGL的函数之前我们需要初始化GLAD。
    int main()
    {
        ...
        //第四步链接顶点属性需要VAO对像储存顶点坐标记录，显卡绘制需要重复生成顶点坐标所以要避免第(一,二,三)步
        unsigned int VAO = create1VAO();
        //第一步把顶点坐标复制到内存（要去掉重复就配合EBO）

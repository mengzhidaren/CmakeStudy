...
//STB_IMAGE_IMPLEMENTATION 这里很重要  main.app里 引用了一次头文件 demo1.cpp定义之后 demo2.cpp就不能在定义了 不然会重复引用
//因为 demo.h文件引用了同一个环境
...
//1. 顶点缓冲对象(Vertex Buffer Objects, VBO)
    ...
    int main() {
        ...
        Shader outShader("04/shader/vertex.vert", "04/shader/fragment.frag");

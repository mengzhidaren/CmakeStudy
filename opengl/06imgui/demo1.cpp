...
// GL_CLAMP_TO_BORDER	超出的坐标为用户指定的边缘颜色。 需要配置颜色glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
    ...
    void openglInit(GLFWwindow *window) {
        // 设置视口
...
//GLAD是用来管理OpenGL的函数指针的 所以在调用任何OpenGL的函数之前我们需要初始化GLAD。
    int main() {
        ...
        Shader outShader("06\\shader\\vertex.vert", "06\\shader\\fragment.frag");
        ...
        vec3 view_tansform = vec3(0.0f, 0.0f, -5.0f);
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            ImGui::SliderInt("width", &SCR_WIDTH, 1.0f, 2000.0f);
            ImGui::SliderInt("height", &SCR_HEIGHT, 1.0f, 2000.0f);
            ImGui::SliderFloat("X", &view_tansform.x, -10.0f, 10.0f);
            ImGui::SliderFloat("Y", &view_tansform.y, -10.0f, 10.0f);
            ImGui::SliderFloat("Z", &view_tansform.z, -10.0f, 10.0f);
            ImGui::End();
            ...
            //将矩阵向我们要进行移动场景的反方向移动
            //Z轴向后 -3.0
            view = translate(view, view_tansform);
